...
namespace EyeSoft.Data.EntityFramework.Caching
{
	...
	/// </summary>
	internal class CachingDataReaderCacheReader : CachingDataReaderBase
	{
		private readonly DbQueryResults queryResults;
		...
		{
			get { return queryResults.Rows.Count > 0; }
		}
		...
		{
			return queryResults.ColumnNames[ordinal];
		}
		...
		{
			if (currentRow < queryResults.Rows.Count)
			{
				var values = queryResults.Rows[currentRow++];
				SetValues(values);
				return true;
			}
