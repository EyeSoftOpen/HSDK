namespace EyeSoft.Test
{
	...
	public class TypeExtensionsTest
	{
		...
		[TestMethod]
		public void CheckFriendlyNameForType()
		{
			typeof(Expression<Func<TestEntity, bool>>).FriendlyName("System.Linq.Expressions", GetType().Namespace)
				.Should().Be.EqualTo("Expression<System.Func<Helpers.TestEntity, bool>>");
		}

		[TestMethod]
		public void CheckFriendlyShortNameForType()
		{
			TypeNameHelper.ShortName("System.Linq.Expressions.Expression<System.Func<EyeSoft.TestEntity, bool>>")
				.Should("ShortName method produces a wrong result.").Be.EqualTo("Expression<Func<TestEntity, bool>>");
		}
		...
		private void CheckIsEnumerableOf<T, TTypeCheck>() where TTypeCheck : class
		{
			...
			var message =
				string.Format("{0} is not recognized as IEnumerable<TInterface>.", type.FriendlyShortName());
